...
import { SpectralClass, MKLuminosity,MorganKeenanName, MorganKeenanClass } from './SpectralClass';
import { Temperature, Luminosity } from './../Units';
import { RGBAColor } from './../Color';


interface ColorInfoPair {
...
type ColorIndex = {[k in MKLuminosity]:Array<ColorValue>} ;
...
export class StarColor {
    ...
    private static ColorTable: ColorInfoPair[] = [
    ...
    private static _buildIndex(ColorTable: ColorInfoPair[]): ColorIndex {
        var o={};
    ...
    private static *_SelectByLuminosity(l:MKLuminosity, table:ColorInfoPair[]) : Iterable<ColorValue> {
        for(let p of table){
            let c: MorganKeenanClass = MorganKeenanClass.Parse(p.key) ;
            if(c && c.luminosity === l){
    ...
    private static Matrix = StarColor._buildIndex(StarColor.ColorTable);

    private static _lookup( source:Array<ColorValue>, temperature:number)
    {
        var low = 0,
        high = source.length;
        ...
        while (low < high) {
            var mid = (low + high) >>> 1;
            if (source[mid].kelvin > temperature) low = mid + 1;
            else high = mid;
        }
        return low;
    }
    ...
    private static _lookupIndexes(luminosity:MKLuminosity, temperature:number): ColorValue[] {
    ...
    public static lookupRgb(luminosity:MKLuminosity,temperature:Temperature|number): RGBAColor {
    
        var kelvin:Temperature = new Temperature(temperature,Temperature.Units.k);
    
        var i:ColorValue[] = this._lookupIndexes(luminosity, kelvin.value);
            
        if (!i || !i.length) return new RGBAColor(0, 0, 0);
        ...
        var k1 = i[1].kelvin;
        var dk = (k1 - k0) ; 
        var c0:RGBAColor = i[0].color;
        if( dk == 0 ) return c0;
        var c1 :RGBAColor = i[1].color;;
        var f:ParametricValue = (kelvin.value - k0) / dk;
